...
(def query [:map {:closed true}
                    [:brand           {:swagger/title         "Brand - either qantas or jetstar"
                                       :swagger/default       "qantas"}              string?]
                    ...
                    [:user_groups     {:swagger/title   "User Groups - QFF Members or Flight Bookers"
                                       :swagger/default "qff_member"}                string?]
...
(def query-with-validation ;; here :fn allows any predicate function to be used
  [:and query
   ...
   [:fn {:error/message "should be: empty string, qff_member, flight_booker or both qff_member and flight_booker"
         :error/path    [:user_groups]}
    (fn [{:keys [user_groups]}] (contains? #{"" "qff_member" "flight_booker" "qff_member,flight_booker" "flight_booker,qff_member"} user_groups))]
...
(def query-for-generation
  [:map {:closed true}
   [:brand           {:swagger/title   "Brand - either qantas or jetstar"
                      :swagger/default "qantas"}                    [:enum "qantas" "jetstar"]]
   [:region_name     {:swagger/title   "Region Name"
                      :swagger/default "Melbourne, VIC, Australia"} [:enum "Melbourne, VIC, Australia" "Sydney, NSW, Australia"]]
   [:adults          {:swagger/title   "Adults"
                      :swagger/default "2"}                         [:enum "1" "2"]]
   [:children        {:optional true
                      :swagger/title   "Children"
                      :swagger/default "0"}                         [:enum "0" "1" "2"]]
   [:infants         {:optional true
                      :swagger/title   "Infants"
                      :swagger/default "0"}                         [:enum "0" "1" "2"]]
   [:check_in        {:swagger/title   "Check In"
                      :swagger/default "2022-07-01"}                [:enum "2021-09-10" "2021-09-11"]]
   [:check_out       {:swagger/title   "Check Out"
                      :swagger/default "2022-07-02"}                [:enum "2021-09-12" "2021-09-13"]]
   [:user_groups     {:swagger/title   "User Groups - QFF Members or Flight Bookers"
                      :swagger/default "qff_member"}                [:enum "qff_member" "flight_booker" "qff_member,flight_booker" ]]
   [:min_star_rating {:optional true
                      :swagger/title   "Minimum Star Rating"
                      :swagger/default 4.5}                         [:and :double [:>= 0] [:<= 5]]]
   [:property_type   {:optional true
                      :swagger/title   "Property Types - one of the property types in the swagger introduction"
                      :swagger/default "Hotels"}                    [:enum "hotels" "motels" "apartments"]]
   [:ne_lng         {:optional true
                     :swagger/title    "North East Longitude"
                     :swagger/default  134}                          valid-longitude?]
   [:ne_lat         {:optional true
